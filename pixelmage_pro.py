...
async def cmd_free(message: types.Message):
    """Бесплатный подарок для новых пользователей"""
    user_id = message.from_user.id
    ...
    if result and result[0] > 0:
        # Уже есть изображения на балансе
        await message.answer(
            "🎁 Вы уже получали бесплатный подарок!\n\n"
            "Но можете купить пакет изображений - это выгодно!",
            reply_markup=get_main_keyboard()
        )
    else:
        # Дарим 1 бесплатное изображение
        await add_balance(user_id, 1, 0)
        
        await message.answer(
            "🎁 <b>БЕСПЛАТНЫЙ ПОДАРОК!</b>\n\n"
            "✅ Вам начислено: <b>1 изображение</b>\n\n"
            "🎨 Что можно сделать:\n"
